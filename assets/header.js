...
class HeaderComponent extends Component {
  ...
  #observeStickyPosition = (alwaysSticky = true) => {
    ...
    if (!this.#stickySentinel || !this.#stickySentinel.isConnected) {
      this.#stickySentinel = document.createElement('div');
      this.#stickySentinel.className = 'header-sticky-sentinel';
      this.#stickySentinel.setAttribute('aria-hidden', 'true');
      this.#stickySentinel.style.cssText = 'position: relative; height: 1px; width: 100%; pointer-events: none;';
      this.insertAdjacentElement('beforebegin', this.#stickySentinel);
    }

    const tolerance = 2;
    const setStickyState = (active) => {
      const next = active ? 'active' : 'inactive';
      if (this.dataset.stickyState === next) return;
      this.dataset.stickyState = next;
    ...
    this.#intersectionObserver = new IntersectionObserver(([entry]) => {
      ...
      if (alwaysSticky) {
        setStickyState((window.scrollY || 0) > tolerance || !isIntersecting);
      } else {
        this.#offscreen = !isIntersecting || this.dataset.stickyState === 'active';
      }
    }, { threshold: 0 });

    this.#intersectionObserver.observe(this.#stickySentinel);

    if (alwaysSticky && !this.#stickyScrollHandler) {
      this.#stickyScrollHandler = () => setStickyState((window.scrollY || 0) > tolerance);
